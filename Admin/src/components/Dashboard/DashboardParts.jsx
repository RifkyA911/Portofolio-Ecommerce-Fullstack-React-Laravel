...
// Layout
// Utils
import { AreaCharts, BarsChart, LineCharts } from "./DashboardCharts.jsx";
import { ReactIcons } from "../../utils/RenderIcons.jsx";
import { Link } from "react-router-dom";
import { CurrencyFormatter, DateFormatter } from "../../utils/Formatter.js";
import { OrderIconStatus } from "../Orders/OrdersTableBody.jsx";
import { MotionTabs } from "../Button.jsx";
import { SkeltonBox } from "../Skelton.jsx";
import RequestAPI from "../../Config/API.jsx";
import { useSelector } from "react-redux";
import TableReview from "../Table/MyTableContent.jsx";
// Data

export const DashboardFirstSection = (props) => {
  // ---- Orders Basic States ----
  ...
  return (
    <>
          ...
          <AreaCharts
            title={
              <>
                ...
                />
                <p className="pl-2 font-roboto-medium">Market Income</p>
              </>
            }
          />
        </div>
      ...
      {!orders ? (
        <SkeltonBox
          className="h-20 lg:h-full w-full lg:w-5/12 rounded-xl"
          count={1}
        />
      ) : (
                        ...
                        {orders.map((order, index) => (
                          ...
                          >
                            <div className="flex flex-col items-center justify-center w-2/12">
                              <OrderIconStatus status={order.status} />
                            ...
                            <div className="flex flex-row items-center w-10/12 ml-3 overflow-hidden text-left justify-between">
                              ...
                              <div className="flex flex-col self-end">
                                ...
                                >
                                  {DateFormatter("Day", order.updated_at)}
                                </small>
                              </div>
                            </div>
                          </li>
...
export const DashboardSecondSection = (props) => {
  // REDUX
  const {
    DarkMode,
    container,
    ...
    <>
      ...
      >
        <LineCharts
          title={
            <>
              <ReactIcons iconName="IoBarChart" className="text-indigo-500" />
        ...
        {/* <ol role="list" className="divide-y divide-slate-200">
          <li className="flex items-center py-2 first:pt-0 last:pb-0">
            <ReactIcons iconName="FaShirt" className="text-indigo-500" />
            <p className="pl-2 font-roboto-medium">Orders Statistic</p>
          </li>
          <li className="flex items-center py-2 first:pt-0 last:pb-0">
            <ReactIcons iconName="FaShirt" className="text-indigo-500" />
            <p className="pl-2 font-roboto-medium">Orders Statistic</p>
